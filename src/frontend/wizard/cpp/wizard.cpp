...
void song(const MasteringUtility::Song &song, bool displayInfo = true)
{
	if (displayInfo)
		std::cout << "\n=== Song Summary ===\n";
	std::cout << "Title: " << song.Title << " | Artist: " << song.Artist << " | Album: " << song.Album
...
bool handleCommandLine(int argc, char **argv, std::filesystem::path &markupPath)
{
	...
	if (conlib.f_boolean("help"))
	{
		std::cout << "Mastering Utility Wizard\n"
		          << "Usage: mastering_wizard [--markupfile=<path>] [--auto] "
		             "[--help]\n\n"
		          << "Options:\n"
		          << "  --markupfile, -f   Path to output Markup file.\n"
		          << "  --auto, -a      Run in automatic mode with default "
		             "settings.\n"
...
void collectAlbumsInteractively(MasteringUtility::Albums &albums)
{
	...
	if (initialAlbumCount > 0)
	{
		...
		DConsole::prompt("Enter the total number of albums to process", albumCount, "Must be a positive integer.",
		                 &albumCount);
	}
	else
	{
	...
	for (int i = 0; i < initialAlbumCount; ++i)
	{
		...
		DConsole::prompt("Enter Relative Path to save songs", albumNewPathStr, "Album save path cannot be empty.",
		                 albumNewPathPtr);
		...
		DConsole::prompt("Enter Relative Path to Album Art", albumAlbumArtStr, "Album art path cannot be empty.",
		                 albumAlbumArtPtr);
		album.AlbumArt = albumAlbumArtStr;

		const std::string *albumArgsPtr = album.arguments.empty() ? &defaultArgs : &album.arguments;
		DConsole::prompt("Enter additional arguments for FFMPEG (if any)", album.arguments, "Unexpected issue",
		                 albumArgsPtr);
		...
		DConsole::prompt("Enter the total number of songs to process in \"" + album.Title + "\"", songCount,
		...
		for (int j = 0; j < initialSongCount; ++j)
		{
			...
			DConsole::prompt("Enter Song Source Filename", songPathStr, "Song source filename cannot be empty.",
			                 songPathPtr);
			...
			DConsole::prompt("Enter additional arguments for FFMPEG (if any)", song.arguments, "Unexpected issue",
			                 songArgsPtr);
		}
		...
		for (int j = initialSongCount; j < songCount; ++j)
		{
			...
			if (!addSong)
			{
				...
				while (response != 'y' && response != 'n' && response != 'a')
					DConsole::prompt("Would you like to add this song to the list? (y/n/a)", response,
		...
		{
			std::cout << "Data for \"" << album.Title
			          << "\" will be kept for now. Run the wizard again to "
			             "remove it.\n";
		}
	}
	...
	for (int i = initialAlbumCount; i < albumCount; ++i)
	{
		...
		int songCount = 0;
		DConsole::prompt("How many songs in \"" + album.Title + "\"?", songCount, "Must be 0 or a positive integer.",
		                 &songCount);
		if (songCount < 0)
			throw std::runtime_error("Song count cannot be negative.");
		...
		DConsole::prompt("Would you like to add this new album to the list? (y/n)", addAlbumResponse,
		                 "Please enter 'y' or 'n'.", &addAlbumResponse);
 ...
 */
void saveMarkupFile(MasteringUtility &masterer, const MasteringUtility::Albums &albums,
...
int main(int argc, char **argv)
{
	...
	try
	{
		...
		{
			std::cout << "Save operation cancelled by user. Exiting without "
			             "saving.\n";
		}
	}
