...
fn preview_song(
    wrapper: &UniquePtr<MasteringUtilWrapper>,
    album_index: usize,
    song_index: usize,
    display_info: bool,
) {
...
fn handle_command_line(
    args: &[String],
    _wrapper: &mut UniquePtr<MasteringUtilWrapper>,
) -> Option<PathBuf> {
    ...
    if help_flag {
        ...
        println!(
            "Usage: {} [--markupfile=<path>] [--auto] [--help]\n",
            args[0]
        );
        println!("Options:");
...
fn collect_albums_interactively(
    wrapper: &mut UniquePtr<MasteringUtilWrapper>,
) -> Result<(), String> {
    ...
    for i in 0..initial_album_count {
        ...
        let copyright = prompt_string(
            "Enter Album Copyright Info",
            Some(if copyright.is_empty() {
                DEFAULT_COPYRIGHT
            } else {
                &copyright
            }),
        ...
        let new_path = prompt_string(
            "Enter Relative Path to save songs",
        ...
        let album_art = prompt_string(
            "Enter Relative Path to Album Art",
            Some(if album_art.is_empty() {
                DEFAULT_PATH_STR
            } else {
                &album_art
            }),
        ...
        let arguments = prompt_string(
            "Enter additional arguments for FFMPEG (if any)",
        ...
        let song_count;
        println!(
            "\nAlbum \"{}\" currently has {} song(s).",
            title, initial_song_count
        );

        song_count = prompt_usize(
            &format!(
                "Enter the total number of songs to process in \"{}\"",
                title
            ),
        ...
        for j in 0..initial_song_count {
            ...
            let song_artist_default = if song_artist.is_empty() {
                wrapper.GetAlbumArtist(i)
            } else {
                song_artist
            };
            ...
            let song_genre_default = if song_genre.is_empty() {
                wrapper.GetAlbumGenre(i)
            } else {
                song_genre
            };
            ...
            let song_year_default = if song_year.is_empty() {
                wrapper.GetAlbumYear(i)
            } else {
                song_year
            };
            ...
            let song_copyright_default = if song_copyright.is_empty() {
                wrapper.GetAlbumCopyright(i)
            } else {
                song_copyright
            };
            let song_copyright =
                prompt_string("Enter Song Copyright Info", Some(&song_copyright_default));
            ...
            let codec = prompt_string(
                "Enter Song Codec (mp3, flac, etc.)",
        ...
        for j in initial_song_count..song_count {
            ...
            println!(
                ...
                j + 1,
                wrapper.GetAlbumTitle(i)
            );

            wrapper
                .pin_mut()
                .SetSongTrackNumber(i, new_song_index, (j + 1) as i32);
            wrapper
                .pin_mut()
                .SetSongID(i, new_song_index, (j + 1) as i32);

            let album_title = wrapper.GetAlbumTitle(i);
            wrapper
                .pin_mut()
                .SetSongAlbum(i, new_song_index, &album_title);
            ...
            let song_copyright = prompt_string(
                "Enter Song Copyright Info",
                Some(&wrapper.GetAlbumCopyright(i)),
            );
            wrapper
                .pin_mut()
                .SetSongCopyright(i, new_song_index, &song_copyright);

            let new_path = prompt_string("Enter New Filename", Some(DEFAULT_PATH_STR));
            wrapper
                .pin_mut()
                .SetSongNewPath(i, new_song_index, &new_path);
            ...
            wrapper
                .pin_mut()
                .SetSongArguments(i, new_song_index, &arguments);
            ...
            if !add_song {
                let response =
                    prompt_char("Would you like to add this song to the list? (y/n/a)", None);
                match response {
                    'a' => {
                        unsafe {
                            AUTO_ADD_SONGS = true;
    ...
    for i in initial_album_count..album_count {
        ...
        wrapper
            .pin_mut()
            .SetAlbumID(new_album_index, (i + 1) as i32);
        wrapper
            .pin_mut()
            .SetAlbumComment(new_album_index, DEFAULT_COMMENT);
        ...
        wrapper
            .pin_mut()
            .SetAlbumArguments(new_album_index, &arguments);

        let song_count_new_album =
            prompt_usize(&format!("How many songs in \"{}\"?", title), Some(0));
        ...
        for j in 0..song_count_new_album {
            ...
            wrapper
                .pin_mut()
                .SetSongTrackNumber(new_album_index, new_song_index, (j + 1) as i32);
            wrapper
                .pin_mut()
                .SetSongID(new_album_index, new_song_index, (j + 1) as i32);

            let album_title = wrapper.GetAlbumTitle(new_album_index);
            wrapper
                .pin_mut()
                .SetSongAlbum(new_album_index, new_song_index, &album_title);

            let path = prompt_string("Enter Song Source Filename", Some(DEFAULT_PATH_STR));
            wrapper
                .pin_mut()
                .SetSongPath(new_album_index, new_song_index, &path);

            let song_title = prompt_string("Enter Song Title", None);
            wrapper
                .pin_mut()
                .SetSongTitle(new_album_index, new_song_index, &song_title);

            let song_artist = prompt_string(
                "Enter Song Artist",
                Some(&wrapper.GetAlbumArtist(new_album_index)),
            );
            wrapper
                .pin_mut()
                .SetSongArtist(new_album_index, new_song_index, &song_artist);

            let song_genre = prompt_string(
                "Enter Song Genre",
                Some(&wrapper.GetAlbumGenre(new_album_index)),
            );
            wrapper
                .pin_mut()
                .SetSongGenre(new_album_index, new_song_index, &song_genre);

            let song_year = prompt_string(
                "Enter Song Year",
                Some(&wrapper.GetAlbumYear(new_album_index)),
            );
            wrapper
                .pin_mut()
                .SetSongYear(new_album_index, new_song_index, &song_year);

            let song_copyright = prompt_string(
                "Enter Song Copyright Info",
                Some(&wrapper.GetAlbumCopyright(new_album_index)),
            );
            wrapper
                .pin_mut()
                .SetSongCopyright(new_album_index, new_song_index, &song_copyright);

            let new_path = prompt_string("Enter New Filename", Some(DEFAULT_PATH_STR));
            wrapper
                .pin_mut()
                .SetSongNewPath(new_album_index, new_song_index, &new_path);

            let codec = prompt_string("Enter Song Codec (mp3, flac, etc.)", Some(DEFAULT_CODEC));
            wrapper
                .pin_mut()
                .SetSongCodec(new_album_index, new_song_index, &codec);
            ...
            wrapper
                .pin_mut()
                .SetSongArguments(new_album_index, new_song_index, &arguments);
            ...
            if !add_song {
                wrapper
                    .pin_mut()
                    .RemoveSong(new_album_index, new_song_index);
            }
        }
...
fn save_markup_file(wrapper: &mut UniquePtr<MasteringUtilWrapper>, markup_path: &Path) {
    ...
    println!(
        "Markup file saved to \"{}\" successfully.",
        markup_path.display()
    );
}
...
fn main() {
    ...
    let result = (|| -> Result<(), String> {
        if markup_path.exists() {
            let path_str = markup_path
                .to_str()
                .ok_or("Invalid markup path for parsing")?;
