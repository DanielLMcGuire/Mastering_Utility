...
bool compareStrings(const std::string &a, const std::string &b,
  ...
  if (a != b) {
    std::cerr << "FAIL: Mismatch in " << fieldName << ": '" << a << "' != '"
              << b << "'\n";
    return false;
  }
...
int main(int argc, char **argv) {
  (void)argc; // Suppress unreferenced parameter warning
  (void)argv; // Suppress unreferenced parameter warning
  DConsole conlib;
  auto start = std::chrono::high_resolution_clock::now(); // start timer
  std::filesystem::path outFile = std::filesystem::temp_directory_path() /
                                  ("tempdir_" + generateRandomString(8)) /
                                  "test.mas";
  ...
  masterer.SaveMarkup(albums, outFile);

  MasteringUtility::Albums inputAlbums;
  masterer.ParseMarkup(outFile, inputAlbums);
  ...
  for (size_t i = 0; i < albums.size(); ++i) {
    ...
    allOk &= compareStrings(std::to_string(orig.ID), std::to_string(parsed.ID),
                            "Album ID");
    allOk &= compareStrings(orig.Title, parsed.Title, "Album Title");
    allOk &= compareStrings(orig.Artist, parsed.Artist, "Album Artist");
    allOk &=
        compareStrings(orig.Copyright, parsed.Copyright, "Album Copyright");
    allOk &= compareStrings(orig.AlbumArt.string(), parsed.AlbumArt.string(),
                            "Album Art");
    allOk &=
        compareStrings(orig.Path.string(), parsed.Path.string(), "Album Path");
    allOk &= compareStrings(orig.NewPath.string(), parsed.NewPath.string(),
                            "Album NewPath");
    ...
    for (size_t j = 0; j < orig.SongsList.size(); ++j) {
      ...
      allOk &= compareStrings(std::to_string(oSong.ID),
                              std::to_string(pSong.ID), "Song ID");
      allOk &= compareStrings(oSong.Title, pSong.Title, "Song Title");
      allOk &= compareStrings(oSong.Artist, pSong.Artist, "Song Artist");
      allOk &=
          compareStrings(std::to_string(oSong.TrackNumber),
                         std::to_string(pSong.TrackNumber), "Song TrackNumber");
      allOk &=
          compareStrings(oSong.Path.string(), pSong.Path.string(), "Song Path");
      allOk &= compareStrings(oSong.NewPath.string(), pSong.NewPath.string(),
                              "Song NewPath");
  ...
  if (allOk) {
    ...
    std::cout << "All metadata matched successfully in " << elapsed.count()
              << " seconds.\n";
    return 0;
  } else {
